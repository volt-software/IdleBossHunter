...
TEST_CASE("company members repository tests") {
    company_members_repository<database_transaction> member_repo{};
    ...
    SECTION("company members inserted correctly" ) {
        ...
        REQUIRE(member2->wage == member.wage);
    }

    SECTION("company members updated correctly" ) {
        auto transaction = db_pool->create_transaction();
        db_user user{};
        user_repo.insert_if_not_exists(user, transaction);
        REQUIRE(user.id > 0);
        ...
        member.wage = 3;
        member_repo.update(member, transaction);
    ...
    SECTION("company members no double insertion" ) {
        auto transaction = db_pool->create_transaction();
        db_user user{};
        user_repo.insert_if_not_exists(user, transaction);
        REQUIRE(user.id > 0);
    ...
    SECTION("get company member by character id" ) {
        auto transaction = db_pool->create_transaction();
        db_user user{};
        user_repo.insert_if_not_exists(user, transaction);
        REQUIRE(user.id > 0);
    ...
    SECTION( "get all character members" ) {
        ...
        REQUIRE(members[0].member_level == member.member_level);
        REQUIRE(members[1].company_id == member2.company_id);
        REQUIRE(members[1].character_id == member2.character_id);
        REQUIRE(members[1].member_level == member2.member_level);
    }
}
