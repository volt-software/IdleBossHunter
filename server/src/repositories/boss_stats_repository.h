...
namespace lotr {
    ...
    public:
        explicit boss_stats_repository(shared_ptr<pool_T> database_pool);
        ...
        optional<db_boss_stat> get(uint64_t id, unique_ptr<transaction_T> const &transaction) const;
        vector<db_boss_stat> get_by_boss_id(uint64_t boss_id, unique_ptr<transaction_T> const &transaction) const;
    private:
