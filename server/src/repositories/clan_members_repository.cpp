...
bool clan_members_repository<transaction_T>::insert(db_clan_member const &member, unique_ptr<transaction_T> const &transaction) const {
    try {
        transaction->execute(fmt::format("INSERT INTO clan_members (clan_id, character_id, member_level) VALUES ({}, {}, {})",
                            member.clan_id, member.character_id, member.member_level));
