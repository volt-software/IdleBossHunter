...
namespace ibh {
    ...
    void handle_set_tax(Server *s, rapidjson::Document const &d, unique_ptr<database_transaction> const &transaction, per_socket_data<WebSocket> *user_data,
        ...
        DESERIALIZE_WITH_PLAYING_CHECK(set_tax_request);
...
#ifdef TEST_CODE
    template void handle_set_tax<custom_server, custom_hdl>(custom_server *s, rapidjson::Document const &d, unique_ptr<database_transaction> const &transaction,
