...
namespace ibh {
    ...
    void handle_reject_application(Server *s, rapidjson::Document const &d, unique_ptr<database_transaction> const &transaction, per_socket_data<WebSocket> *user_data,
        ...
        DESERIALIZE_WITH_PLAYING_CHECK(reject_application_request);

        q.enqueue(make_unique<reject_application_message>(user_data->connection_id, msg->applicant_id));
    }

    template void handle_reject_application<server, websocketpp::connection_hdl>(server *s, rapidjson::Document const &d, unique_ptr<database_transaction> const &transaction,
...
#ifdef TEST_CODE
    template void handle_reject_application<custom_server, custom_hdl>(custom_server *s, rapidjson::Document const &d, unique_ptr<database_transaction> const &transaction,
