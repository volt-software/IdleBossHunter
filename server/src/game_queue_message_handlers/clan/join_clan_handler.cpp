    ...
    bool handle_join_clan(queue_message* msg, entt::registry& es, outward_queues& outward_queue, unique_ptr<database_transaction> const &transaction) {
        ...
        for(auto entity : pc_view) {
            ...
            if(gold_it->second < 10'000) {
                auto new_err_msg = make_unique<join_clan_response>("Not enough gold, need 10,000 to create clan.");
                outward_queue.enqueue({pc.connection_id, move(new_err_msg)});
                return false;
            }
            ...
            db_clan new_clan{0, join_msg->clan_name, {}, {}};
            if(!clan_repo.insert(new_clan, subtransaction)) {
                auto new_err_msg = make_unique<join_clan_response>("Clan name already exists");
                outward_queue.enqueue({pc.connection_id, move(new_err_msg)});
                return false;
            }
            ...
            es.assign<clan_component>(clan_entt, new_clan.id, join_msg->clan_name, vector<clan_member_component>{{pc.id, CLAN_ADMIN}}, clan_stats);
            ...
            spdlog::trace("[{}] created clan {} for pc {} for connection id {}", __FUNCTION__, join_msg->clan_name, pc.name, pc.connection_id);
