...
string play_character_request::serialize() const {
    spdlog::trace("[play_character_request] type {}", type);
...
unique_ptr<play_character_request> play_character_request::deserialize(rapidjson::Document const &d) {
    ...
    return make_unique<play_character_request>(d["slot"].GetUint());
}
